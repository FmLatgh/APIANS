...
/datum/round_event/ghost_role/prisoner/spawn_role()
	...
	if(result != SUCCESSFUL_SPAWN)
		return result
	priority_announce("i hired these death row inmates to look at you (they are not to be killed)", "United States of Galentopia", SSstation.announcer.get_rand_report_sound())
