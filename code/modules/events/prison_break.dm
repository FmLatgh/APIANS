...
/datum/round_event/grey_tide/announce(fake)
	if(areasToOpen && areasToOpen.len > 0)
		priority_announce("uh oh it's pretty [severity], hope you can deal with the greytiders", "United States of Galentopia", SSstation.announcer.get_rand_alert_sound())
	else
